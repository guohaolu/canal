package com.alibaba.otter.canal.client.adapter.ewayt.clickhouse;
...
import com.alibaba.otter.canal.client.adapter.ewayt.clickhouse.config.ConfigLoader;
...
import com.alibaba.otter.canal.client.adapter.ewayt.clickhouse.monitor.ClickHouseConfigMonitor;
import com.alibaba.otter.canal.client.adapter.ewayt.clickhouse.service.ClickHouseBatchSyncService;
import com.alibaba.otter.canal.client.adapter.ewayt.clickhouse.service.ClickHouseEtlService;
import com.alibaba.otter.canal.client.adapter.ewayt.clickhouse.service.ClickHouseMirrorDbBatchSyncService;
 ...
 */
@SPI("ewayt-clickhouse")
public class EwaytClickHouseAdapter implements OuterAdapter {

    private static final Logger                         logger = LoggerFactory.getLogger(EwaytClickHouseAdapter.class);
    ...
    /**
     * 将同步配置添加到缓存中
     * 此方法根据配置名称和映射配置对象，决定将配置添加到普通的配置缓存中还是镜像数据库配置缓存中
     *
     * @param configName 配置名称，用于在缓存中标识特定的配置
     * @param mappingConfig 映射配置对象，包含数据库映射和目的地等信息
     */
    private void addSyncConfigToCache(String configName, MappingConfig mappingConfig) {
        ...
        if (!mappingConfig.getDbMapping().getMirrorDb()) {
            ...
            // 使用计算若不存在则放入的方法，确保键对应的值是并发HashMap
